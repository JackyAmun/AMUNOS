...
int main(void) {
    ...
    int win = sys_gui_win(20, 20, 440, 340, "控件演示 v6.6");
    if (win < 0) { sys_gui_leave(); return -1; }

    /* 菜单栏 (v6.6): File(F) / Edit(E) / Help(H) */
    ...
    int b_exit  = sys_gui_btn(win, 340, 60, "退出");

    /* 输入框 */
    sys_gui_lbl(win,  20, 100, "输入:");
    int ed = sys_gui_edit(win, 80, 100, 240);

    /* 复选框 (v6.6) */
    sys_gui_lbl(win, 20, 130, "复选:");
    int chk_b = sys_gui_check(win, 80, 130, "粗体");
    int chk_i = sys_gui_check(win, 160, 130, "斜体");
    sys_gui_check_set(win, chk_b, 1);

    /* 单选 (v6.6) */
    sys_gui_lbl(win, 20, 158, "字号:");
    int r_s = sys_gui_radio(win, 80, 158, "小");
    int r_m = sys_gui_radio(win, 130, 158, "中");
    int r_l = sys_gui_radio(win, 180, 158, "大");
    sys_gui_check_set(win, r_m, 1);                    /* 默认选中"中" */

    /* 列表 */
    sys_gui_lbl(win,  20, 190, "列表(↑↓+Enter):");
    int li = sys_gui_list(win, 160, 190, 260, 100);
    for (int i = 0; i < NCITIES; i++) sys_gui_list_set(win, li, cities[i]);

    /* 状态标签 (反馈) */
    int st = sys_gui_lbl(win, 20, 310, "状态: 等待 (TAB 切焦点 / Alt+F 开菜单)");
        ...
        for (int i = 0; i < n; i++) {
            if (ev[i].type == GEV_CLICK) {
                ...
                if (ev[i].ctl == mb) {
                    ...
                    appdec(s, it);
                    if (m == mfile && it == 3) { sys_gui_leave(); return 0; } /* 退出 */
                    if (m == mfile && it == 0) sys_gui_wnd_text(win, ed, "新建文件");
                    if (m == mfile && it == 1) sys_gui_wnd_text(win, ed, "打开文件");
                    if (m == medit) {
                    ...
                    if (m == mhelp && it == 0)
                        sys_gui_wnd_text(win, st, "关于: AMUNOS Classic GUI 0.1 v6.6");
                    if (ev[i].ch == ((mfile<<8)|3)) continue;
